...
namespace LoadProjectData
{
    public class CrGroupConnectorRegistrationDao
    {
        ...
        public bool Exists(int groupconnectorid, int registrationid)
        {
            var rc = false;
            var count = 0;
            const string query = "SELECT count(*) FROM dbo.cr_GroupConnectorRegistrations WHERE groupconnector_id = @GroupConnectorId and registration_id = @RegistrationId";
        ...
        public int Insert(CrGroupConnectorRegistration groupconnectorregistration)
        {
            ...
            var rc = -1;
            const string query = "INSERT INTO dbo.cr_GroupConnectorRegistrations (GroupConnector_ID,Registration_ID,Domain_ID,Role_ID) " +
                                 "OUTPUT INSERTED.GroupConnectorRegistration_ID " +
        ...
        public int GetGroupConnectorRegistrationId(int groupconnectorid, int registrationid)
        {
            ...
            const string query = "SELECT groupconnectorregistration_id FROM dbo.cr_GroupConnectorRegistrations WHERE groupconnector_id = @GroupConnectorId and registration_id = @RegistrationId";
